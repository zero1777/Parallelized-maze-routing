...
void horizontalSweep() {
    for (int x = 0; x < H; x++) {
        vector<int> tmpWeight(vWeight[x]);
        for (int i = 1; i <= ceil(log2(W)); i++) {
            for (int j = 0; j < W; j++) {
                if (j % int(pow(2, i)) >= pow(2, i - 1)) {
                    int idx = j - (j % int(pow(2, i - 1))) - 1;
                    if (gridMap[x][idx]->cost != INT_MAX && gridMap[x][j]->cost > gridMap[x][idx]->cost + tmpWeight[j - 1]) {
                        gridMap[x][j]->cost = gridMap[x][idx]->cost + tmpWeight[j - 1];
                        gridMap[x][j]->prev = transTo1D(x, j - 1);
                        changed = true;
                    }
                    if (j >= 2) {
                        tmpWeight[j - 1] += tmpWeight[j - 2];
                    }
                }
            }
        }
        ...
        for (int i = 1; i <= ceil(log2(W)); i++) {
            for (int j = W - 1; j >= 0; j--) {
                ...
                if (invert_j % int(pow(2, i)) >= pow(2, i - 1)) {
                    ...
                    if (gridMap[x][idx]->cost != INT_MAX && gridMap[x][j]->cost > gridMap[x][idx]->cost + tmpWeight[j]) {
                        gridMap[x][j]->cost = gridMap[x][idx]->cost + tmpWeight[j];
                        gridMap[x][j]->prev = transTo1D(x, j + 1);
                        changed = true;
                    }
                    if (j < W - 1) {
                        tmpWeight[j] += tmpWeight[j + 1];
                    }
                }
            }
        }
...
void verticalSweep() {
    for (int y = 0; y < W; y++) {
        vector<int> tmpWeight(H - 1);
        for (int i = 0; i < H - 1; i++) tmpWeight[i] = hWeight[i][y];
        for (int i = 1; i <= ceil(log2(H)); i++) {
            for (int j = 0; j < H; j++) {
                if (j % int(pow(2, i)) >= pow(2, i - 1)) {
                    int idx = j - (j % int(pow(2, i - 1))) - 1;
                    if (gridMap[idx][y]->cost != INT_MAX && gridMap[j][y]->cost > gridMap[idx][y]->cost + tmpWeight[j - 1]) {
                        gridMap[j][y]->cost = gridMap[idx][y]->cost + tmpWeight[j - 1];
                        gridMap[j][y]->prev = transTo1D(j - 1, y);
                        changed = true;
                    }
                    if (j >= 2) {
                        tmpWeight[j - 1] += tmpWeight[j - 2];
        ...
        for (int i = 1; i <= ceil(log2(H)); i++) {
            for (int j = H - 1; j >= 0 ; j--) {
                ...
                if (invert_j % int(pow(2, i)) >= pow(2, i - 1)) {
                    ...
                    if (gridMap[idx][y]->cost != INT_MAX && gridMap[j][y]->cost > gridMap[idx][y]->cost + tmpWeight[j]) {
                        gridMap[j][y]->cost = gridMap[idx][y]->cost + tmpWeight[j];
                        gridMap[j][y]->prev = transTo1D(j + 1, y);
                        changed = true;
                    }
                    if (j < H - 1) {
