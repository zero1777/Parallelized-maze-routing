...
void horizontalSweep() {
    for (int x = 0; x < H; x++) {
        for (int y = 1; y < W; y++) {
            if (gridMap[x][y - 1]->cost != INT_MAX && gridMap[x][y]->cost > gridMap[x][y - 1]->cost + vWeight[x][y - 1]) {
                gridMap[x][y]->cost = gridMap[x][y - 1]->cost + vWeight[x][y - 1];
                gridMap[x][y]->prev = transTo1D(x, y - 1);
                changed = true;
            }
        }
        for (int y = W - 2; y >= 0; y--) {
            if (gridMap[x][y + 1]->cost != INT_MAX && gridMap[x][y]->cost > gridMap[x][y + 1]->cost + vWeight[x][y]) {
                gridMap[x][y]->cost = gridMap[x][y + 1]->cost + vWeight[x][y];
                gridMap[x][y]->prev = transTo1D(x, y + 1);
                changed = true;
            }
        }
...
void verticalSweep() {
    for (int y = 0; y < W; y++) {
        for (int x = 1; x < H; x++) {
            if (gridMap[x - 1][y]->cost != INT_MAX && gridMap[x][y]->cost > gridMap[x - 1][y]->cost + hWeight[x - 1][y]) {
                gridMap[x][y]->cost = gridMap[x - 1][y]->cost + hWeight[x - 1][y];
                gridMap[x][y]->prev = transTo1D(x - 1, y);
                changed = true;
            }
        }
        for (int x = H - 2; x >= 0; x--) {
            if (gridMap[x + 1][y]->cost != INT_MAX && gridMap[x][y]->cost > gridMap[x + 1][y]->cost + hWeight[x][y]) {
                gridMap[x][y]->cost = gridMap[x + 1][y]->cost + hWeight[x][y];
                gridMap[x][y]->prev = transTo1D(x + 1, y);
                changed = true;
            }
        }
