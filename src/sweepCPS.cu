    ...
    for (int i = 1; i <= ceil(log2f(W)); i++) {
        for (int j = id; j < W; j += stride) {
            ...
            // if (invert_j % int(pow(2, i)) >= pow(2, i - 1)) {
                ...
                int idx = W - 1 - (invert_j - (invert_j & (a1 - 1)) - 1);
                int partialCost = 0;
    ...
    for (int i = 1; i <= ceil(log2f(H)); i++) {
        for (int j = id; j < H; j += stride) {
            ...
            // if (invert_j % int(pow(2, i)) >= pow(2, i - 1)) {
                ...
                int idx = H - 1 - (invert_j - (invert_j & (a1 - 1)) - 1);
                int partialCost = 0;
    ...
    for (int i = 0; i < numOfPins - 1; i++) {
        ...
        while (true) {
            ...
            *changed = 0;
            ...
            // cudaDeviceSynchronize();
            verticalSweep1<<<numOfBlocks, numOfThreads>>>(H, W, dgridMap, dhWeight, dchanged);
            cudaDeviceSynchronize();
            // verticalSweep2<<<numOfBlocks, numOfThreads>>>(H, W, dgridMap, dhWeight, dchanged);
            // cudaDeviceSynchronize();
